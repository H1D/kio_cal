...
def redactor_upload(request, upload_to=None, form_class=ImageForm, response=lambda name, url: url):
    ...
    if form.is_valid():
        ...
        path = os.path.join(upload_to or  UPLOAD_PATH, file.name.encode('ascii', 'ignore'))
        real_path = default_storage.save(path, file)
        real_path = real_path.replace(settings.MEDIA_ROOT, '')
        url = ''.join((settings.MEDIA_URL[:-1], real_path))
        return HttpResponse(response(file.name, url ))
