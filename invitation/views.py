    ...
    **Context:**
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            new_user.groups.add(Group.objects.get_or_create(name='author')[0])
            new_user.save()

            article = invitation.article
            article.authors.add(new_user)
            article.save()
            ...
            messages.success(request,u'''
                %s, спасибо за регистрацию! Теперь можно приступить к статье.
            '''%new_user.first_name)
            return HttpResponseRedirect(article.get_admin_url())
    else:
...
def invite_author(request):
    try:
        ...
        invitation = Invitation.objects.invite(user=request.user,
                                email=email,
                                article = article)
